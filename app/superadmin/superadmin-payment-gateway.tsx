...
type GatewayForm = {
    ...
    isTestMode: boolean;
    keyId: string;
    keySecret: string;      // left blank on load — write-only field
    webhookSecret: string;  // left blank on load — write-only field
    currency: string;
    companyName: string;
    companyLogo: string;
    description: string;
...
export default function PaymentGatewaySettings() {
    ...
    // Tracks whether a secret was already configured server-side (so we can
    // show "leave blank to keep current" instead of "required").
    ...
    const loadConfig = async () => {
        ...
        try {
            ...
            const loaded: GatewayForm = {
                isEnabled: data.isEnabled ?? false,
                isTestMode: data.isTestMode ?? true,
                keyId: data.keyId ?? '',
                keySecret: '', // never prefilled — backend only returns a mask
                webhookSecret: '',
                currency: data.currency || 'INR',
                companyName: data.companyName ?? '',
                companyLogo: data.companyLogo ?? '',
                description: data.description ?? EMPTY_FORM.description,
            };
            setHasExistingSecret(!!data.keySecret); // mask string means "something is set"
            setHasExistingWebhook(!!data.webhookSecret);
            setConfig(loaded);
            setOriginal(loaded);
        } catch (e: any) {
            if (e.status !== 404) {
                Alert.alert('Error', e.message || 'Could not load payment gateway settings.');
            }
        } finally {
            setLoading(false);
        }
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        setSaving(true);
                        try {
                            // webhookSecret is optional server-side — only send it if
                            // the user typed a new value, otherwise leave it untouched.
                            ...
                            const saved = await saveGatewaySettings(payload);
                            const merged: GatewayForm = {
                                ...config,
                                keySecret: '',
                                webhookSecret: '',
                            };
                            setHasExistingSecret(!!saved.keySecret);
                            setHasExistingWebhook(!!saved.webhookSecret);
                            setConfig(merged);
                            setOriginal(merged);
                            Alert.alert(
                                'Saved ✅',
    ...
    const update = (key: keyof GatewayForm, value: any) => {
        ...
        if (!isTest) {
            Alert.alert(
                ...
                [
                    ...
                    { text: 'Switch to Live', style: 'destructive', onPress: () => update('isTestMode', false) }
                ]
            );
        } else {
        ...
        <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
                ...
                <View style={[
                    ...
                    <Ionicons name={config.isEnabled ? 'checkmark-circle' : 'pause-circle'} size={22} color={config.isEnabled ? '#2e7d32' : '#e67e22'} />
                    <View style={{ flex: 1, marginLeft: 10 }}>
                        ...
                        <Text style={styles.statusSubtitle}>
                            {config.isEnabled ? `Running in ${config.isTestMode ? 'TEST' : 'LIVE'} mode` : 'Configure and enable to accept payments'}
                        </Text>
                    </View>
                    <Switch value={config.isEnabled} onValueChange={v => update('isEnabled', v)} trackColor={{ false: '#ddd', true: '#a5d6a7' }} thumbColor={config.isEnabled ? '#2e7d32' : '#f4f3f4'} />
                </View>

                <View style={styles.infoBanner}>
                    <Ionicons name="information-circle" size={18} color="#1565c0" />
                    <Text style={styles.infoText}>
                        These settings are saved to your database. The app reads them at checkout time. Keep your Key Secret safe — never share it publicly.
                    </Text>
                </View>

                <Text style={styles.sectionTitle}>Mode</Text>
                <View style={styles.modeRow}>
                    <TouchableOpacity style={[styles.modeBtn, config.isTestMode && styles.modeBtnActive]} onPress={() => handleModeToggle(true)}>
                    ...
                    <TouchableOpacity style={[styles.modeBtn, !config.isTestMode && styles.modeBtnLive]} onPress={() => handleModeToggle(false)}>
                ...
                {config.isTestMode ? (
                    <Text style={styles.modeHint}>🧪 Test mode: Use test cards — no real money is charged. Get test keys from Razorpay Dashboard → Settings → API Keys → Test Keys.</Text>
                ) : (
                    <Text style={[styles.modeHint, { color: '#d32f2f' }]}>⚠️ Live mode: Real money will be charged. Make sure your Razorpay account is fully verified before going live.</Text>
                )}
                ...
                <View style={styles.fieldCard}>
                    ...
                    <TextInput style={styles.input} value={config.keyId} onChangeText={v => update('keyId', v)} placeholder={config.isTestMode ? 'rzp_test_xxxxxxxxxx' : 'rzp_live_xxxxxxxxxx'} placeholderTextColor="#bbb" autoCapitalize="none" autoCorrect={false} />
                    <Text style={styles.hint}>{config.isTestMode ? 'Must start with rzp_test_' : 'Must start with rzp_live_'}</Text>
                </View>
                ...
                <View style={styles.fieldCard}>
                    ...
                    <TextInput
                        ...
                        placeholder={hasExistingSecret ? 'Set — type a new value to change it' : 'Your Razorpay Key Secret'}
                    ...
                    <Text style={styles.hint}>
                        {hasExistingSecret
                            ? 'For security this is never shown back to you. Leave blank to keep the current secret.'
                            : 'Razorpay Dashboard → Settings → API Keys → Copy Secret.'}
                    </Text>
                </View>
                ...
                <View style={styles.fieldCard}>
                    ...
                    <TextInput
                        ...
                        placeholder={hasExistingWebhook ? 'Set — type a new value to change it' : 'Webhook secret (for server-side verification)'}
                        placeholderTextColor="#bbb"
                        secureTextEntry={!showWebhook}
                        autoCapitalize="none"
                        autoCorrect={false}
                    />
                    <Text style={styles.hint}>Only needed if you set up a webhook endpoint for payment verification.</Text>
                </View>
                ...
                <View style={styles.fieldCard}>
                    ...
                    <TextInput style={styles.input} value={config.companyName} onChangeText={v => update('companyName', v)} placeholder="e.g. TechCRM Solutions" placeholderTextColor="#bbb" />
                ...
                <View style={styles.fieldCard}>
                    ...
                    <TextInput style={styles.input} value={config.companyLogo} onChangeText={v => update('companyLogo', v)} placeholder="https://your-logo-url.png" placeholderTextColor="#bbb" keyboardType="url" autoCapitalize="none" />
                    <Text style={styles.hint}>Logo shown in Razorpay popup. Upload to Supabase Storage and paste URL.</Text>
                </View>
                ...
                <View style={styles.fieldCard}>
                    ...
                    <TextInput style={styles.input} value={config.description} onChangeText={v => update('description', v)} placeholder="e.g. CRM/LMS Subscription Payment" placeholderTextColor="#bbb" />
                </View>
                ...
                <View style={styles.fieldCard}>
                    ...
                    <View style={styles.currencyRow}>
                        {['INR', 'USD', 'EUR'].map(c => (
                            <TouchableOpacity key={c} style={[styles.currencyChip, config.currency === c && styles.currencyChipActive]} onPress={() => update('currency', c)}>
                ...
                <View style={styles.previewCard}>
                    ...
                    <PreviewRow icon="lock-closed" color="#d32f2f" label="Key Secret" value={hasExistingSecret ? '••••••••••••' : '— Not set'} />
                    ...
                    <PreviewRow icon={config.isEnabled ? 'checkmark-circle' : 'pause-circle'} color={config.isEnabled ? '#2e7d32' : '#e67e22'} label="Status" value={config.isEnabled ? 'ENABLED ✅' : 'DISABLED'} />
                </View>
                ...
                <View style={styles.howToCard}>
                    ...
                    <TouchableOpacity style={styles.razorpayLink} onPress={() => Linking.openURL('https://dashboard.razorpay.com/app/keys')}>
                        <Ionicons name="open-outline" size={14} color="#3b5998" />
                        <Text style={styles.razorpayLinkText}>Open Razorpay Dashboard</Text>
                    </TouchableOpacity>
                </View>

                <TouchableOpacity style={[styles.saveBtn, (!isDirty || saving) && styles.saveBtnDisabled]} onPress={handleSave} disabled={!isDirty || saving}>
                    {saving ? <ActivityIndicator color="white" size="small" /> : <Ionicons name="save" size={20} color="white" style={{ marginRight: 8 }} />}
                    <Text style={styles.saveBtnText}>{saving ? 'Saving...' : isDirty ? 'Save Settings' : 'No Changes'}</Text>
                </TouchableOpacity>

                {!isDirty && !saving && <Text style={styles.noChangeHint}>Make a change above to enable saving.</Text>}
...
const styles = StyleSheet.create({
    ...
    infoBanner: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#e3f2fd', borderRadius: 12, padding: 12, gap: 8, marginBottom: 6 },
    infoText: { flex: 1, fontSize: 12, color: '#1565c0', lineHeight: 18 },
    sectionTitle: { fontSize: 12, fontWeight: '700', color: '#999', textTransform: 'uppercase', letterSpacing: 0.8, marginTop: 20, marginBottom: 6 },
    sectionSubtitle: { fontSize: 12, color: '#aaa', marginBottom: 10, marginTop: -4 },
    modeRow: { flexDirection: 'row', gap: 10, marginBottom: 8 },
    modeBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 12, borderRadius: 12, backgroundColor: 'white', borderWidth: 1.5, borderColor: '#ddd', elevation: 1 },
    ...
    fieldCard: { backgroundColor: 'white', borderRadius: 14, padding: 14, marginBottom: 10, elevation: 1, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, shadowOffset: { width: 0, height: 1 } },
    ...
    input: { borderWidth: 1.5, borderColor: '#e0e0e0', borderRadius: 10, paddingHorizontal: 13, paddingVertical: 11, fontSize: 14, color: '#222', backgroundColor: '#fafafa', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace' },
    ...
    previewCard: { backgroundColor: 'white', borderRadius: 14, padding: 14, gap: 10, elevation: 1 },
    ...
    howToCard: { backgroundColor: '#f0f3ff', borderRadius: 14, padding: 14, marginTop: 16, borderWidth: 1, borderColor: '#dde3f5' },
    ...
    razorpayLink: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10, backgroundColor: 'white', padding: 10, borderRadius: 8, alignSelf: 'flex-start' },
    razorpayLinkText: { fontSize: 13, color: '#3b5998', fontWeight: '600' },
    saveBtn: { flexDirection: 'row', backgroundColor: '#d32f2f', padding: 16, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginTop: 24, elevation: 3 },
